...
import ObligationItem from '../obligations/ObligationItem';
...
function SubcategoryBlock({
  ...
  function cancelAdd() {
    setShowAdd(false);
    setNewName(''); setNewAmount(''); setNewDueDay(''); setNewRecurring(true);
  }
            ...
            <>
              ...
              {!isUncategorized && !confirmDelete && (
                <div className="opacity-0 group-hover/hdr:opacity-100 flex gap-1 transition-opacity">
                  <button
                    onClick={() => { setEditNameInput(subcat.name); setEditingName(true); }}
        ...
        {showAdd ? (
          <div className="p-3 flex flex-col gap-2" style={items.length > 0 ? { borderTop: '1px solid #2a2d3e' } : {}}>
            ...
            <div className="flex rounded-lg overflow-hidden border" style={{ borderColor: '#2a2d3e' }}>
              <button type="button" onClick={() => setNewRecurring(true)}
                className="flex-1 py-1.5 text-xs font-semibold transition-colors"
                style={{ backgroundColor: newRecurring ? '#6366f1' : 'transparent', color: newRecurring ? '#f1f5f9' : '#64748b' }}>
                Recurring
              </button>
              <button type="button" onClick={() => setNewRecurring(false)}
                className="flex-1 py-1.5 text-xs font-semibold transition-colors border-l"
                style={{ borderColor: '#2a2d3e', backgroundColor: !newRecurring ? '#6366f1' : 'transparent', color: !newRecurring ? '#f1f5f9' : '#64748b' }}>
                One-time
              </button>
            </div>
            <div className="flex gap-2">
...
export default function ObligationsSection({
  ...
  const totalCommitted = obligations.reduce((s, o) => s + (o.amount || 0), 0);
  const assignedThisMonth = obligations
    .filter(o => o.assignedMonth === cm)
    .reduce((s, o) => s + (o.assignedAmount || 0), 0);
  const gap = Math.max(0, totalCommitted - assignedThisMonth);

  const subcatIds = new Set(subcategories.map(s => s.id));
  const uncategorized = obligations.filter(o => !o.subcategoryId || !subcatIds.has(o.subcategoryId));
      ...
      <div className="flex items-center gap-4 pb-3 mb-5" style={{ borderBottom: '1px solid #2a2d3e' }}>
        ...
        {obligations.length > 0 && (
          <span className="text-xs tabular-nums" style={{ color: '#64748b' }}>
            {formatCurrency(totalCommitted)} total
            {gap > 0 && <span style={{ color: '#f59e0b' }}> · {formatCurrency(gap)} gap</span>}
          </span>
        )}
      </div>
      ...
      <div className="flex flex-col gap-5">
        {subcategories.map(subcat => (
          <SubcategoryBlock
            ...
            cm={cm}
            onAdd={(name, amount, dueDay, recurring) => addObligation(name, amount, dueDay, recurring, subcat.id)}
            onUpdateItem={updateObligation}
            onDeleteItem={deleteObligation}
            onAssignItem={assignObligation}
        ...
        {(uncategorized.length > 0 || subcategories.length === 0) && (
          <SubcategoryBlock
            ...
            allSubcats={subcategories}
            cm={cm}
            onAdd={(name, amount, dueDay, recurring) => addObligation(name, amount, dueDay, recurring, '')}
            onUpdateItem={updateObligation}
            onDeleteItem={deleteObligation}
            onAssignItem={assignObligation}
            onTogglePaid={togglePaid}
        ...
        {showNewSubcat ? (
          <div className="flex gap-2">
            <input
              ...
              style={{ borderColor: '#2a2d3e', color: '#f1f5f9' }}
              placeholder="Subcategory name (e.g. Bills, Subscriptions)"
              value={newSubcatName}
