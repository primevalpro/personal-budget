...
function SubcategoryBlock({
  ...
  const total = items.reduce((s, g) => s + (g.targetAmount || 0), 0);

  function cancelAdd() {
    setShowAdd(false);
    setNewCategory(''); setNewTarget('');
  }

  async function handleAdd() {
    const category = newCategory.trim();
    const target = Number(newTarget);
    if (!category || isNaN(target) || target < 0) return;
    await onAdd(category, target);
    cancelAdd();
  }
...
export default function GoalsSection({
  ...
  const totalAssigned = goals.reduce((s, g) => s + (g.assignedAmount || 0), 0);
  const gap = Math.max(0, totalTarget - totalAssigned);

  const subcatIds = new Set(subcategories.map(s => s.id));
  const uncategorized = goals.filter(g => !g.subcategoryId || !subcatIds.has(g.subcategoryId));
      ...
      <div className="flex flex-col gap-5">
        {subcategories.map(subcat => (
          <SubcategoryBlock
            ...
            onAdd={(category, target) => addGoal(category, target, subcat.id)}
            onUpdateItem={updateGoal}
            onDeleteItem={deleteGoal}
            onAssignItem={assignGoal}
            onAddSpend={addSpend}
        ...
        {(uncategorized.length > 0 || subcategories.length === 0) && (
          <SubcategoryBlock
            ...
            items={uncategorized}
            allSubcats={subcategories}
            onAdd={(category, target) => addGoal(category, target, '')}
            onUpdateItem={updateGoal}
            onDeleteItem={deleteGoal}
            onAssignItem={assignGoal}
            onAddSpend={addSpend}
        ...
        {showNewSubcat ? (
          <div className="flex gap-2">
            <input
              ...
              style={{ borderColor: '#2a2d3e', color: '#f1f5f9' }}
              placeholder="Subcategory name (e.g. Essentials, Discretionary)"
              value={newSubcatName}
